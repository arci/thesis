...
The main problem encountered was due to the presence of multiple persistence provider in the classpath, expecially for the one provided within the \textit{jpa4azure} library.

\noindent At runtime, even if Kundera was the provider defined in the \textit{persistence.xml} file, the instantiated provider was the one of \textit{jpa4azure}. Even if all the other specific JPA implementation were removed from the classpath, \textit{jpa4azure} cannot be removed since it was not providing only the necessary code for the JPA interface, but also utility code to ease the interaction with other Azure services such as Blob and Queues, code that is actually used inside the CPIM implemetation of those services for Azure thus removing \textit{jpa4azure} will cause the loss of those functionality.
