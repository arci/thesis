...
\newparagraph SCHEMA HIGH LEVEL FUNCTIONING
...
\end{itemize}
\noindent User is not directly invoking method on the provider entity manager but interacts with the persistence provider through the \texttt{CloudEntityManager} class which implements the JPA \texttt{EntityManager} interface. Without the support for the migration system the \texttt{CloudEntityManager} should delegate every method call to the provider entity manager.
To integrate migration and synchronization logic, the method mentioned above should contains a little amount of logic shown in the snippet of code \ref{code:isMigrating} taking as example the persist operation.

\begin{algorithm}[h]
  \begin{algorithmic}[1]
    \Procedure{EntityManager.persist}{$object$}
	\If{MigrationManager.isMigrating()}
		\State $statement \gets$ buildStatement($object$)
		\State MigrationManager.propagate($statement$)
   	\Else
...
\newparagraph The Datastore extension has been modified to persist relationships as \texttt{kind\textunderscore key-identifier} instead of the \texttt{Key} instance.
...
\newparagraph The Azure Table extension has been modified to
